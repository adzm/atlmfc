...
HINSTANCE AFXAPI AfxFindResourceHandle(LPCTSTR lpszName, LPCTSTR lpszType)
{
	...
	for (pDLL = pModuleState->m_libraryList; pDLL != NULL; pDLL = pDLL->m_pNextDLL)
	{
		if (!pDLL->m_bSystem && pDLL->m_hResource != NULL)
		{
			...
			hInstCache.Add(pDLL->m_hResource);
		}
	}
	AfxUnlockGlobals(CRIT_DYNLINKLIST);
...
extern BOOL g_fLoadingResourcesForMFCDLL;
...
BOOL WINAPI DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID)
{
	try
	{
		if (dwReason == DLL_PROCESS_ATTACH)
		{
			...
			// get the path for our module, so we can narrow load of localization DLL to only that same location.  if
			// we're unable to get the path, the localization DLL is not loaded (so MFC resources fall back to English).
