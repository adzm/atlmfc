...
namespace ATL
{
typedef struct LcidToLocaleName
{
	LCID      lcid;
	wchar_t*  localeName;
} LCIDTOLOCALENAME;

typedef struct LocaleNameIndex
{
	wchar_t*  name;
	int       index;
} LOCALENAMEINDEX;
...
const LCIDTOLOCALENAME LcidToLocaleNameTable [] =  {
	{ 0x0001, L"ar"         },
	{ 0x0002, L"bg"         },
	{ 0x0003, L"ca"         },
	{ 0x0004, L"zh-CHS"     },
...
// API Reference at http://msdn.microsoft.com/en-us/goglobal/bb896001.aspx
// The table is sorted to improve search performance.
const LOCALENAMEINDEX LocaleNameToIndexTable [] = 
{
	{ L""           , 66  },
	{ L"af"         , 44  },
	{ L"af-za"      , 113 },
...
LCID _AtlDownlevelLocaleNameToLCID(LPCWSTR localeName)
{
	...
	int index = GetTableIndexFromLocaleName(localeName);
...
int _AtlDownlevelLCIDToLocaleName(LCID lcid, LPWSTR outLocaleName, int cchLocaleName)
{
	...
	int index = GetTableIndexFromLcid(lcid);
	if (index < 0)
	{
		return 0;
	}

	wchar_t* buffer = LcidToLocaleNameTable[index].localeName;
...
int _AtlLCMapStringEx(_In_opt_ LPCWSTR lpLocaleName, _In_ DWORD dwMapFlags, _In_ LPCWSTR lpSrcStr, _In_ int cchSrc, _Out_opt_ LPWSTR lpDestStr, _In_ int cchDest, _In_opt_ LPNLSVERSIONINFO lpVersionInformation, _In_opt_ LPVOID lpReserved, _In_opt_ LPARAM sortHandle)
{
	...
	IFDYNAMICGETCACHEDFUNCTION(L"kernel32.dll", LCMapStringEx, pfLCMapStringEx)
	{
